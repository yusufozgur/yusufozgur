...
- **Git** vs **Jujutsu**: Git is ubiquitous but its full capabilities are hard to master and non-basic workflows are not ergonomic. A modern version control system like jujutsu provides better primitives and gives freedom of expression and ease of navigation to the developer with respect to their repositories.
...
- **LaTeX** vs **Typst**: I believe the LaTeX has singlehandedly became the biggest obstacle towards wider adoption of typesetting languages as opposed to using WYSIWYG editors due to the convoluted ecosystem and slow toolchains it has accumulated over the years. In contrast, Typst provides an easy to learn start with a cohesive and clear docs, immensely helping beginners quickly become productive, especially when paired with its webUI. In addition, the functional programming approach used in its domain specific language models the needs of a document typesetting system with great success. Typst has become one of my favorite tools over the years, and it should be used any-time a solution with a pdf generation end-result is needed, be it automated reporting systems, documentations, theses, and papers. 
...
- **Deep Learning Frameworks**: New learners should learn pytorch, as deep learning research have moved on from tensorflow and other old technologies. JAX is an interesting up and coming alternative. Meta-frameworks such as pytorch lightning could be useful for certain usecases, but such high level abstractions could also hinder your understanding in deep learning research and their bloated nature may make your codebase harder to handle for simpler usecases.
- **Shell Scripting**: Ditch bash,zsh,fish. Embrace Nushell.
- **Operating Systems**: Windows is the devil, apple is a walled garden, no freedom. Linux is a thorny path leading to freedom, but the only blessed way to use linux is using NixOS. Learning curve is only steep because we are thaught imperative paradigms are the only way, but nix's declarative environments will save your soul.
